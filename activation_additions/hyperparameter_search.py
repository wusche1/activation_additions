...
from activation_additions import prompt_utils, utils, metrics, hook_utils
...
def add_actads_to_model(
    model: HookedTransformer, ActAds: Optional[List[ActivationAddition]] = None
) -> None:
    """
    Adds a list of activation vectors to a model if provided.

    This function modifies the input model by adding the provided activation
    vectors if they are provided. If ActAds is None, the function does nothing.

    Args:
        model (HookedTransformer): The model to which activations should be added.
        ActAds (Optional[List[ActivationAddition]]): List of activation vectors to be added. If None, no action is taken.
    ...
    Example:
        ...
        activations = [ActivationAddition(...), ActivationAddition(...)]
        add_actads_to_model(model, activations)
    """
    ...
    for act_name, hooks in hook_fns.items():
...
def conditional_perplexity(
    ...
    """
    Computes the conditional perplexity of the completion tokens given the prompt tokens for a given model.

    This function calculates the negative sum of the log probabilities of each completion token when conditioned on the
    provided prompt tokens using the provided model. Higher values indicate less probable completions, and lower values
    indicate more probable completions.

    Args:
        model (HookedTransformer): The model used to compute log probabilities.
        prompt_tokens (torch.Tensor): Tensor of tokens representing the prompt/context for the completion.
        completion_tokens (torch.Tensor): Tensor of tokens representing the completion text.

    Returns:
        float: The computed conditional perplexity value.

    Example:
        prompt = torch.tensor([...])   # Example tensor for the prompt
        completion = torch.tensor([...])   # Example tensor for the completion
        perplexity = conditional_perplexity(model_instance, prompt, completion)

    Notes:
        - The function assumes that the dimensions of the prompt_tokens and completion_tokens tensors are compatible
          for concatenation along dimension 1.
        - Lower perplexity values are generally better as they indicate higher likelihoods, but the absolute value
          can vary based on the model and dataset.
    """
    ...
    completion_logprobs = metric["logprob_actual_next_token"].array[0][
...
def completion_perplexities(
    ...
    perplexity_on_wanted = [
        conditional_perplexity(model, prompt, completion)
...
def layer_coefficient_gridsearch(
    ...
    wanted_completion_tokens = [
        model.to_tokens(wanted_completion)[:, 1:]
        for wanted_completion in wanted_completions
    ]
    unwanted_completion_tokens = [
        model.to_tokens(unwanted_completion)[:, 1:]
        ...
        for coefficient in coefficient_list:
            ActAds = [
                prompt_utils.ActivationAddition(
                    coeff=prompt_weighting * coefficient, act_name=layer, prompt=prompt
                )
...
def plot_matrices_with_pareto(df: pd.DataFrame, plot_pareto_points: bool = True) -> None:
    """
